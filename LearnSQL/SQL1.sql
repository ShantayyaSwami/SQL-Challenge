...
-- use sql keyword as col use word in [] (sql servef specific) or ` `(mysql) to mark it as normal col name
create database Scaler if does not exists
use Scaler

create table bank_details(age tinyint, job varchar(30), marital varchar(30), education varchar(40), `default` varchar(30), bal int, housing varchar(30),
loan varchar(30), contact varchar(10), `day` tinyint, `month` varchar(15), duration int, campaign int, pdays int, previous int, poutcome varchar(30),
y varchar(30))
...
select `default`, `day`, `month` from bank_details

--select top 5 * from bank_details   sql server
select * from bank_details limit 5

select * from bank_details where age >=  60 and job = 'retired'

select * from bank_details where (education ='unknown' or marital ='single') and bal < 500 
...
-- sum of bal
select sum(bal) as SUM_OF_BAL from bank_details
-- avg of bal
select avg(bal) as AVERAGE_BAL from bank_details
--min and max bal
select min(bal) as MIN_BAL, max(bal) as MAX_BAL from bank_details
--records who has min bal
select * from bank_details where bal in (select min(bal) from bank_details)
select * from bank_details where bal = (select min(bal) from bank_details)
select * from bank_details order by bal limit 1

-- having loan records
select count(*) from bank_details where loan = 'yes'
--avg bal of folks who are admin
select avg(bal) from bank_details where job = 'admin.'
--without job whose age < 45
select * from bank_details where job = 'unknown' and age < 45
--eductaion is primary but jobless
select * from bank_details where education = 'primary' and job = 'unknown'
--accnt have negative bal
select * from bank_details where bal < 0
-- no house and no bal
select * from bank_details where housing = 'no' and bal <= 0


create database Employee;
use employee
create database TCS;
create table employee(EMP_name varchar(50), designation varchar(30), emp_address varchar(30))
insert into employee values('shantayya', 'Senior DevOps Engineer', 'Pune')
select * from employee;
show variables like 'sql_safe_updates'     --sql safe is on so it throws error on update and delete queries without where clause 
set sql_safe_updates = 0  		-- disabling the sql safe mode

delete from employee
truncate table employee
drop table employee
insert into Employee values('Rahul', 'Middleware Admin', 'Bihar'), ('Sachin', 'Salesforce dev', 'Solapur')