...
create table activity1(player_id int, device_id int, event_date date, games_played int); 
...
select player_id, device_id from activity1 where event_date in (select min(event_date) from activity1 group by player_id);
-- or
select player_id,device_id from (
select player_id,device_id,row_number() over (partition by player_id order by event_date) as rownum from activity1) test where rownum = 1;
